document.addEventListener("click", function (event) {
    ...
    if (event.target.classList.contains("delete-permission-icon")) {
        if (confirm('Voulez-vous vraiment supprimer ce rôle ?')) {
            const deleteUrl = event.target.getAttribute('data-delete-permission-url');
            ...
            .then(data => {
                if (data.success) {
                    ...
                    const permissionRow = event.target.closest('tr'); // Trouver la ligne du rôle
                    if (permissionRow) {
                        permissionRow.remove(); // Supprimer la ligne du DOM
                    }
                } else if (data.danger) {
