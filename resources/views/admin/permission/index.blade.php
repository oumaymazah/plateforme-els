            ...
            <div class="card border-0 shadow-sm h-100 bg-white border-start border-primary border-3">
                <div class="card-body d-flex justify-content-between align-items-center py-2">
                    <div>
                        ...
                        <h3 class="mb-0 fw-bold text-primary">{{ count($permissions) }}</h3>
                    </div>
                ...
                <div class="input-group">
                    ...
                    <select class="form-select filter-select" id="role-filter-permission">
                        ...
                        @foreach($roles as $role)
                            <option value="{{ $role->name }}" {{ isset($selectedRole) && $selectedRole == $role->name ? 'selected' : '' }}>
                                {{ $role->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <tr >
                        ...
                        <td>
                            <div class="d-flex flex-wrap gap-1">
                                @forelse($permission->roles ?? [] as $role)
                                    ...
                                    @case('super-admin')
                                        <span class="badge bg-danger text-white" style="background-color: #900 !important;">{{ $role->name }}</span>
                                        @break
                                        @case('admin')
                                            <span class="badge  bg-danger bg-opacity-75">{{ $role->name }}</span>
                                            @break
                                        @case('professeur')
                                            <span class="badge  bg-primary bg-opacity-75">{{ $role->name }}</span>
                                            @break
                                        @case('etudiant')
                                            <span class="badge  bg-info bg-opacity-75">{{ $role->name }}</span>
                                            @break
                                        @default
                                            <span class="badge  bg-success bg-opacity-75">{{ $role->name }}</span>
                                    @endswitch
                                @empty
                        ...
                        <td>
                            <span class="badge bg-light text-dark border">
                                {{ $permission->users_count }} utilisateurs
                            </span>
                        </td>
                        <td class="text-center">
                            <div class="dropdown">
                                <button class="btn btn-sm btn-light dropdown-toggle" type="button"
                                        id="dropdownMenuButton-{{ $permission->id }}" data-bs-toggle="dropdown"
                                        aria-expanded="false">
                                ...
                                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton-{{ $permission->id }}">
                                    <li>
                                        <a class="dropdown-item edit-permission" href="#"
                                           data-edit-permission-url="{{ route('admin.permissions.edit', $permission) }}">
